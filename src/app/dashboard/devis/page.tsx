  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Link href="/dashboard/devis/new" style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'linear-gradient(135deg, #C9A96E, #B8944F)', color: '#1A1410', padding: '10px 18px', borderRadius: '12px', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>
          <Plus size={14} /> Nouveau devis
        </Link>
      </div>

      {error && (
        <div style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.25)', color: '#F87171', padding: '10px 16px', borderRadius: '10px', fontSize: '13px', display: 'flex', justifyContent: 'space-between' }}>
          {error}
          <button onClick={() => setError(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#F87171', fontSize: '16px' }}>✕</button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <button onClick={load} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.5)', padding: '8px 14px', borderRadius: '10px', fontSize: '13px', cursor: 'pointer' }}>
      ...
      {loading ? (
        ...
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)' }}>Aucun devis trouvé.</p>
      ) : (
              ...
              {devis.map(d => (
                <tr key={d.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  <td style={{ padding: '12px 16px' }}>
                    <Link href={`/dashboard/devis/${d.id}`} style={{ fontFamily: 'monospace', color: '#C9A96E', textDecoration: 'none', fontSize: '13px', fontWeight: 500 }}>
                      {d.number}
                    </Link>
                  </td>
                  <td style={{ padding: '12px 16px', color: 'rgba(255,255,255,0.8)' }}>{d.clients?.name}</td>
                  <td style={{ padding: '12px 16px', color: 'rgba(255,255,255,0.4)' }}>{new Date(d.created_at).toLocaleDateString('fr-FR')}</td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 500, color: 'rgba(255,255,255,0.9)' }}>{d.total_ttc.toFixed(2)} MAD</td>
                  <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                    <span style={{ display: 'inline-block', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 500, ...(STATUS_COLORS[d.status] ?? {}) }}>
                      {STATUS_LABELS[d.status] ?? d.status}
                    </span>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '10px' }}>
                      <a href={`/api/devis/${d.id}/pdf`} target="_blank" rel="noreferrer" title="Télécharger PDF" style={{ color: 'rgba(255,255,255,0.4)', display: 'flex' }}>
                        <Download size={14} />
                      </a>
