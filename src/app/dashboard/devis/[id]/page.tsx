...
const STATUS_COLORS: Record<string, React.CSSProperties> = {
  ...
  expired:  { background: 'rgba(251,146,60,0.15)',  color: '#FB923C' },
}
...
export default function DevisDetailPage({ params }: { params: { id: string } }) {
  ...
  async function handleDelete() {
    ...
    if (res.ok) router.push('/dashboard/devis')
    else { setError('Impossible de supprimer ce devis.'); setActionLoading(null) }
  }
  ...
  function startEdit() {
    ...
    setEditItems(devis.items.map(i => ({ id: crypto.randomUUID(), description: i.description, quantity: Number(i.quantity), unit_price: Number(i.unit_price) })))
    setIsEditing(true)
  }
  ...
  async function handleSave() {
    ...
    const res = await fetch(`/api/devis/${params.id}`, {
      ...
      body: JSON.stringify({ tva_rate: editTvaRate, valid_until: editValidUntil || null, notes: editNotes || null, items: validItems.map(i => ({ description: i.description, quantity: i.quantity, unit_price: i.unit_price })) }),
    })
    ...
    <div style={{ maxWidth: '760px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', marginBottom: '4px' }}>
            <Link href="/dashboard/devis" style={{ color: 'rgba(201,169,110,0.7)', textDecoration: 'none' }}>Devis</Link> / {devis.number}
          </p>
          <h1 style={{ fontFamily: 'monospace', fontSize: '22px', fontWeight: 600, color: 'rgba(255,255,255,0.9)' }}>{devis.number}</h1>
          <span style={{ display: 'inline-block', marginTop: '6px', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 500, ...(STATUS_COLORS[devis.status] ?? {}) }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          {!isEditing && (
            <>
              <a href={`/api/devis/${devis.id}/pdf`} target="_blank" rel="noreferrer" style={{ ...btnSecondary, textDecoration: 'none' }}><Download size={13} /> PDF</a>
              ...
              {devis.status === 'sent' && (
                <button type="button" onClick={() => action('reject')} disabled={!!actionLoading} style={{ ...btnSecondary, color: '#F87171' }}><XCircle size={13} /> Refuser</button>
              )}
              {['draft', 'sent'].includes(devis.status) && (
                <button type="button" onClick={handleConvert} disabled={!!actionLoading} style={{ ...btnSecondary, color: '#4ADE80' }}><ArrowRightLeft size={13} /> Convertir</button>
              )}
              <button type="button" onClick={handleDuplicate} disabled={!!actionLoading} style={btnSecondary}><Copy size={13} /> Dupliquer</button>
              {devis.status === 'draft' && (
                <button type="button" onClick={handleDelete} disabled={!!actionLoading} style={{ ...btnSecondary, color: '#F87171' }}><Trash2 size={13} /> Supprimer</button>
              )}
            </>
          )}
        </div>
      ...
      {isEditing && (
        <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <h2 style={{ fontSize: '15px', fontWeight: 500, color: 'rgba(255,255,255,0.9)' }}>Modifier le devis</h2>
      ...
      <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', fontSize: '13px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <div>
            ...
            <p style={{ color: 'rgba(255,255,255,0.8)' }}>{new Date(devis.created_at).toLocaleDateString('fr-FR')}</p>
          </div>
          {devis.valid_until && (
            <div>
              <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '2px' }}>Valable jusqu&apos;au</p>
              <p style={{ color: 'rgba(255,255,255,0.8)' }}>{new Date(devis.valid_until).toLocaleDateString('fr-FR')}</p>
            </div>
          )}
          <div>
            <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '2px' }}>TVA</p>
            <p style={{ color: 'rgba(255,255,255,0.8)' }}>{Number(devis.tva_rate)}%</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '13px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '24px', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '8px' }}>
            ...
            <span style={{ width: '112px', textAlign: 'right', fontWeight: 700, color: '#C9A96E', fontSize: '15px' }}>{devis.total_ttc.toFixed(2)} MAD</span>
          </div>
        </div>
      </div>
