...
const STATUS_STYLES: Record<string, React.CSSProperties> = {
  ...
  paid:      { background: '#E7F6EC',  color: '#1C9950' },
  cancelled: { background: '#FBECEC', color: '#C94F4F' },
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <h1 style={{ fontFamily: 'serif', fontSize: '28px', fontWeight: 300, color: '#382227', marginTop: '4px' }}>Factures</h1>
        </div>
      ...
      {error && (
        <div style={{ background: '#F0CCCC', border: '1px solid rgba(248,113,113,0.25)', color: '#C94F4F', padding: '10px 16px', borderRadius: '12px', fontSize: '13px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>{error}</span>
          <button type="button" onClick={() => setError(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#C94F4F', marginLeft: '12px', fontSize: '16px', lineHeight: 1 }}>✕</button>
        </div>
      )}
      ...
      {summary && !loading && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
          <div style={{ background: '#FFFFFF', border: '1px solid #FFFFFF', borderRadius: '16px', padding: '20px', textAlign: 'center' }}>
            <p style={{ fontSize: '10px', color: '#9A8288', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '6px' }}>Total HT</p>
            <p style={{ fontSize: '18px', fontWeight: 600, color: '#432B31' }}>{summary.subtotal_ht.toFixed(2)} MAD</p>
          </div>
          <div style={{ background: '#FFFFFF', border: '1px solid #FFFFFF', borderRadius: '16px', padding: '20px', textAlign: 'center' }}>
            <p style={{ fontSize: '10px', color: '#9A8288', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '6px' }}>Total TVA</p>
            <p style={{ fontSize: '18px', fontWeight: 600, color: '#432B31' }}>{summary.tva_amount.toFixed(2)} MAD</p>
          </div>
          <div style={{ background: '#F7E9E6', border: '1px solid #E8C7CE', borderRadius: '16px', padding: '20px', textAlign: 'center' }}>
            <p style={{ fontSize: '10px', color: 'rgba(226, 167, 181,0.6)', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '6px' }}>Total TTC</p>
            <p style={{ fontSize: '18px', fontWeight: 700, color: '#8E4457' }}>{summary.total_ttc.toFixed(2)} MAD</p>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <button type="button" onClick={load} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#FFFFFF', border: '1px solid #EEDCD7', color: '#6A4A52', padding: '8px 14px', borderRadius: '10px', fontSize: '13px', cursor: 'pointer' }}>
      ...
      {payModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#FFFFFF', borderRadius: '20px', padding: '28px', width: '320px', border: '1px solid #EEDCD7', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <h2 style={{ fontSize: '18px', fontFamily: 'serif', fontWeight: 300, color: '#382227' }}>Marquer comme payé</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '11px', color: '#8A6E74', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '6px' }}>Mode de paiement</label>
                <div ref={payMethodRef} style={{ position: 'relative' }}>
                  <button type="button" onClick={() => setPayMethodOpen(o => !o)}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#FFFFFF', border: '1px solid #EEDCD7', borderRadius: '10px', color: '#382227', padding: '8px 12px', fontSize: '13px', outline: 'none', cursor: 'pointer' }}>
                    <span>{{ cash: 'Espèces', card: 'Carte', transfer: 'Virement' }[payModal.paymentMethod]}</span>
                    <ChevronDown size={13} style={{ flexShrink: 0, color: '#8A6E74', transform: payMethodOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }} />
                  </button>
                  {payMethodOpen && (
                    <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: '#FFFFFF', border: '1px solid #EEDCD7', borderRadius: '10px', zIndex: 60 }}>
                      {(['cash', 'card', 'transfer'] as const).map((v, i) => (
                        <button key={v} type="button"
                          onClick={() => { setPayModal(prev => prev ? { ...prev, paymentMethod: v } : prev); setPayMethodOpen(false) }}
                          style={{ width: '100%', textAlign: 'left', padding: '9px 12px', fontSize: '13px', background: payModal.paymentMethod === v ? '#F7E9E6' : 'none', border: 'none', borderTop: i > 0 ? '1px solid #F7E9E6' : 'none', cursor: 'pointer', color: payModal.paymentMethod === v ? '#8E4457' : '#432B31', borderRadius: i === 0 ? '10px 10px 0 0' : i === 2 ? '0 0 10px 10px' : '0' }}>
              ...
              <div>
                <label htmlFor="pay-amount" style={{ display: 'block', fontSize: '11px', color: '#8A6E74', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '6px' }}>Montant payé (MAD)</label>
                <input id="pay-amount" type="number" min="0" step="0.01" value={payModal.paidAmount}
                  ...
                  style={{ width: '100%', background: '#FFFFFF', border: '1px solid #EEDCD7', borderRadius: '10px', color: '#382227', padding: '8px 12px', fontSize: '13px', outline: 'none' }} />
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
              <button type="button" onClick={() => setPayModal(null)}
                style={{ padding: '9px 18px', background: '#FFFFFF', border: '1px solid #EEDCD7', color: '#7E6469', borderRadius: '10px', fontSize: '13px', cursor: 'pointer' }}>
                Annuler
              </button>
              <button type="button" onClick={handleMarkPaid} disabled={!!actionLoading}
                style={{ padding: '9px 18px', background: 'linear-gradient(135deg, #A85D70, #7E4452)', color: '#FFFFFF', borderRadius: '10px', fontSize: '13px', fontWeight: 600, border: 'none', cursor: 'pointer' }}>
                Confirmer
              </button>
            </div>
          </div>
      ...
      {loading ? (
        ...
        <p style={{ color: '#9A8288', fontSize: '14px' }}>Aucune facture trouvée.</p>
      ) : (
        <div style={{ background: '#FFFFFF', border: '1px solid #FFFFFF', borderRadius: '16px', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            ...
            <tbody>
              {factures.map((f, i) => (
                <tr key={f.id} style={{ borderBottom: i < factures.length - 1 ? '1px solid #FFFFFF' : 'none' }}>
                  <td style={{ padding: '12px 14px' }}>
                    <Link href={`/dashboard/factures/${f.id}`} style={{ fontFamily: 'monospace', color: '#8E4457', textDecoration: 'none', fontSize: '13px' }}>
                      {f.number}
                    </Link>
                  </td>
                  <td style={{ padding: '12px 14px', color: '#432B31' }}>{f.clients?.name}</td>
                  <td style={{ padding: '12px 14px', color: '#8A6E74' }}>{new Date(f.created_at).toLocaleDateString('fr-FR')}</td>
                  <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: 600, color: '#432B31' }}>{f.total_ttc.toFixed(2)} MAD</td>
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
                      <a href={`/api/factures/${f.id}/pdf`} target="_blank" rel="noreferrer" title="Télécharger PDF" style={{ color: '#9A8288', display: 'flex' }}>
                        <Download size={14} />
                      </a>
