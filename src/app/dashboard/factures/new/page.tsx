    ...
    <div style={{ maxWidth: '760px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        ...
        <h1 style={{ fontFamily: 'serif', fontSize: '28px', fontWeight: 300, color: '#382227', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Receipt size={22} style={{ color: '#8E4457' }} /> Nouvelle facture
        </h1>
      </div>

      {error && (
        <div style={{ background: '#FBECEC', border: '1px solid rgba(248,113,113,0.25)', color: '#C94F4F', padding: '10px 16px', borderRadius: '10px', fontSize: '13px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span>{error}</span>
          <button type="button" onClick={() => setError(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#C94F4F', fontSize: '16px', marginLeft: '16px' }}>&times;</button>
        </div>
      )}
