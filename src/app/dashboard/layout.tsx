      ...
      <div style={{ minHeight: '100vh', background: '#FAF3EE' }}>
        ...
        <aside
          ...
          style={{
            ...
            background: 'linear-gradient(180deg, #2B1B1E 0%, #241519 50%, #1D1114 100%)',
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '24px 14px', width: '100%' }}>
            <div style={{ width: '40px', height: '40px', flexShrink: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(145deg, rgba(226, 167, 181,0.2), rgba(226, 167, 181,0.08))', border: '1px solid rgba(226, 167, 181,0.3)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.1), 0 2px 12px rgba(0,0,0,0.3)' }}>
              <span style={{ fontFamily: 'serif', fontStyle: 'italic', fontSize: '14px', fontWeight: 600, color: '#E2A7B5', letterSpacing: '0.05em' }}>BS</span>
            </div>
            <div className="sidebar-text">
              <p style={{ fontFamily: 'serif', fontStyle: 'italic', fontSize: '13px', color: '#E2A7B5', lineHeight: 1.2 }}>Brazilian Studio</p>
              <p style={{ fontSize: '9px', letterSpacing: '0.25em', color: 'rgba(226, 167, 181,0.4)', textTransform: 'uppercase' }}>Rabat</p>
            </div>
          </div>
            ...
            {navGroups.map((group, gi) => (
              <div key={group.label}>
                <div style={{ padding: '16px 12px 6px' }}>
                  <p className="sidebar-text" style={{ fontSize: '9px', textTransform: 'uppercase', letterSpacing: '0.2em', fontWeight: 500, color: 'rgba(226, 167, 181,0.35)' }}>{group.label}</p>
                </div>
                  ...
                  return (
                    <div key={href} style={{ position: 'relative' }}>
                      <Link href={href} title={label}
                        style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%', padding: '8px 10px', borderRadius: '12px', marginBottom: '2px', textDecoration: 'none', background: active ? 'rgba(226, 167, 181,0.1)' : 'transparent', color: active ? '#E2A7B5' : 'rgba(255,255,255,0.45)', border: active ? '1px solid rgba(226, 167, 181,0.15)' : '1px solid transparent', transition: 'all 0.15s' }}
          ...
          <div style={{ width: '100%', padding: '0 8px 20px' }}>
            <div style={{ margin: '0 12px 8px', height: '1px', background: 'linear-gradient(90deg, transparent, rgba(226, 167, 181,0.15), transparent)' }} />
            <Link href="/" title="Voir le site"
              style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%', padding: '8px 10px', borderRadius: '12px', textDecoration: 'none', color: 'rgba(255,255,255,0.35)', transition: 'all 0.15s' }}
            ...
            >
              ...
              <span className="nav-link-label" style={{ fontSize: '13px', fontWeight: 500 }}>Voir le site</span>
            </Link>
        ...
        {/* ── Mobile: More sheet ── */}
        {moreOpen && (
          <div className="more-sheet" style={{ position: 'fixed', bottom: '72px', left: 0, right: 0, zIndex: 50, background: '#2A191D', borderTop: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px 20px 0 0', padding: '16px 16px 8px', boxShadow: '0 -8px 32px rgba(0,0,0,0.4)' }}>
              ...
              {allSecondaryItems.map(({ href, label, icon: Icon, badge }) => {
                ...
                return (
                  <Link key={href} href={href} onClick={() => setMoreOpen(false)}
                    style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 14px', borderRadius: '12px', textDecoration: 'none', background: active ? 'rgba(226, 167, 181,0.12)' : 'rgba(255,255,255,0.05)', border: active ? '1px solid rgba(226, 167, 181,0.2)' : '1px solid rgba(255,255,255,0.06)', color: active ? '#E2A7B5' : 'rgba(255,255,255,0.75)' }}>
                    <Icon size={16} />
        ...
        <nav className="mobile-bottom-bar" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 40, background: '#2A191D', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', height: '64px', paddingBottom: 'env(safe-area-inset-bottom)', boxShadow: '0 -4px 24px rgba(0,0,0,0.3)' }}>
          {bottomTabs.map(({ href, label, icon: Icon, highlight }) => {
            ...
            if (highlight) {
              return (
                <Link key={href} href={href}
                  ...
                  <div style={{ width: '44px', height: '44px', borderRadius: '14px', background: 'linear-gradient(135deg, #E2A7B5, #C98FA0)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(226, 167, 181,0.35)', marginTop: '-16px' }}>
                    <Icon size={20} style={{ color: '#2B1B1E' }} />
                  </div>
                </Link>
              )
            }
            return (
              <Link key={href} href={href}
                style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '3px', textDecoration: 'none', color: active ? '#E2A7B5' : 'rgba(255,255,255,0.35)', paddingTop: '6px' }}>
                <Icon size={20} />
          ...
          <button onClick={() => setMoreOpen(o => !o)}
            style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '3px', background: 'none', border: 'none', cursor: 'pointer', color: moreOpen ? '#E2A7B5' : 'rgba(255,255,255,0.35)', paddingTop: '6px' }}>
